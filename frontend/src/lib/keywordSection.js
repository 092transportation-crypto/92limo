...
const SITE = { brand: "92 Limo Service", airports: "BWI, DCA, IAD and PHL", phone: "(877) 609-1919" };
...
// Best-effort place name for a landing page: "Bowie, MD · Prince George's County"
// -> "Bowie, MD"; otherwise the H1 with the service words stripped.
const titleCase = (s) =>
  s === s.toUpperCase()
    ? s.toLowerCase().replace(/(^|[\s-])([a-z])/g, (m, a, b) => a + b.toUpperCase()).replace(/\b(Md|Dc|Va|Pa|De|Bwi|Dca|Iad|Phl)\b/g, (m) => m.toUpperCase())
    : s;
export const placeName = (d) => {
  const eyebrow = (d.eyebrow || "").split(/[·•|]/)[0].trim();
  if (eyebrow && !/service|transportation|transfer|limo|\bcar\b|route|race day|game day|concert|event|airport/i.test(eyebrow)) return titleCase(eyebrow);
  return (d.h1 || "").replace(/\b(limo|limousine|car|chauffeur|black car|shuttle|executive|luxury|transportation|service|rental|transfers?|private)\b/gi, "").replace(/\s+/g, " ").trim() || "the area";
};
